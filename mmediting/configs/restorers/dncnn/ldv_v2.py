...
model = dict(
    ...
    pixel_loss=dict(type='MSELoss', loss_weight=1., reduction='mean')  # loss_weight = .5 in the paper; see form. (1)
)
...
data = dict(
    workers_per_gpu=8,
    train_dataloader=dict(samples_per_gpu=8, drop_last=True),  # batch size = 128 in the paper; here we use 32
...
# learning policy
total_iters = 500000
gamma = math.exp(math.log(1e-3) / total_iters)  # 1e-1 -> 1e-4: 1e-4 = 1e-1 * gamma^(total_iters)
lr_config = dict(
    policy='Exp',
    by_epoch=False,
    gamma=gamma
