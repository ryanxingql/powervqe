...
class BasicVSR(BasicRestorer):
    ...
    def __init__(
        ...
        self.if_accum = if_accum
        self.accum_mul = accum_mul
    ...
    def train_step(self, data_batch, optimizer):
        ...
        if not self.if_accum:
            # optimize
            optimizer['generator'].zero_grad()
            loss.backward()
            optimizer['generator'].step()
        else:  # take self.accum_mul=3 as an example
            if self.step_counter % self.accum_mul == 0:  # 0, 3, ...
                optimizer['generator'].zero_grad()
            loss /= self.accum_mul  # 0, 1, 2, 3, ...
            loss.backward()
            if (self.step_counter + 1) % self.accum_mul == 0:  # 2, 5, ...
                optimizer['generator'].step()
