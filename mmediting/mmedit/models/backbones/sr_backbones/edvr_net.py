...
@BACKBONES.register_module()
class EDVRNetQE(nn.Module):
    """EDVR network structure for video quality enhancement.
    ...
    def forward(self, x):
        ...
        dh = h - (h // self.ds_mul) * self.ds_mul
        dw = w - (w // self.ds_mul) * self.ds_mul
        if_pad = False
        if dh != 0 or dw != 0:
            if_pad = True
        ...
        #     'The height and width of inputs should be a multiple of 4, '
        #     f'but got {h} and {w}.')
        _, _, h_up, w_up = x.size()

        # down-sample input
        x_ds = self.lrelu(self.conv_ds1(x))
        x_ds = self.lrelu(self.conv_ds2(x_ds))

        # extract LR features
        # L1
        # l1_feat = self.lrelu(self.conv_first(x_ds))
        l1_feat = self.feature_extraction(x_ds)
        # L2
        l2_feat = self.feat_l2_conv2(self.feat_l2_conv1(l1_feat))
        # L3
        l3_feat = self.feat_l3_conv2(self.feat_l3_conv1(l2_feat))
