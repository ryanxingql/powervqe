...
class RDNQE(nn.Module):
    """RDN model for single image quality enhancement.
    ...
    def __init__(self,
        ...
        # shallow feature extraction
        self.sfe1 = nn.Conv2d(
            in_channels, mid_channels, kernel_size=3, padding=3 // 2, stride=2)
        self.sfe2 = nn.Conv2d(
            mid_channels,
            mid_channels,
            kernel_size=3,
    ...
    def forward(self, x):
        ...
        x = self.gff(torch.cat(local_features, 1)) + sfe2  # ryanxingql
        # global residual learning
        x = self.upscale(x)
        x = self.output(x)
